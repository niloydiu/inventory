...
export async function getAllLocations(token, activeOnly = false) {
  try {
    ...
    const data = await apiClient.get(LOCATIONS_ENDPOINTS.BASE, params, token);
...
export async function getLocationById(id, token) {
  try {
    const data = await apiClient.get(LOCATIONS_ENDPOINTS.BY_ID(id), {}, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
...
export async function createLocation(locationData, token) {
  try {
    const data = await apiClient.post(LOCATIONS_ENDPOINTS.BASE, locationData, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
