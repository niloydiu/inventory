...
export async function getAllLivestock(token, params = {}) {
  try {
    const response = await apiClient.get(LIVESTOCK_ENDPOINTS.BASE, params, token);
...
export async function getLivestockById(id, token) {
  try {
    const response = await apiClient.get(LIVESTOCK_ENDPOINTS.BY_ID(id), {}, token);
    const data = response.data || response;
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
...
export async function createLivestock(livestockData, token) {
  try {
    const response = await apiClient.post(LIVESTOCK_ENDPOINTS.BASE, livestockData, token);
    const data = response.data || response;
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
...
export async function getHealthReport(token) {
  try {
    const response = await apiClient.get(LIVESTOCK_ENDPOINTS.HEALTH_REPORT, {}, token);
    const data = response.data || response;
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
...
export async function getSpeciesSummary(token) {
  try {
    const response = await apiClient.get(LIVESTOCK_ENDPOINTS.SPECIES_SUMMARY, {}, token);
    const data = response.data || response;
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
