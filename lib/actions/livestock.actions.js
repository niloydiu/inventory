...
export async function getAllLivestock(token, params = {}) {
  try {
    const data = await apiClient.get(LIVESTOCK_ENDPOINTS.BASE, params, token);
...
export async function getLivestockById(id, token) {
  try {
    const data = await apiClient.get(LIVESTOCK_ENDPOINTS.BY_ID(id), {}, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
...
export async function createLivestock(livestockData, token) {
  try {
    const data = await apiClient.post(LIVESTOCK_ENDPOINTS.BASE, livestockData, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
...
export async function getHealthReport(token) {
  try {
    const data = await apiClient.get(LIVESTOCK_ENDPOINTS.HEALTH_REPORT, {}, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
...
export async function getSpeciesSummary(token) {
  try {
    const data = await apiClient.get(LIVESTOCK_ENDPOINTS.SPECIES_SUMMARY, {}, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
