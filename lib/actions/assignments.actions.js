...
export async function getAllAssignments(token, filters = {}) {
  try {
    const data = await apiClient.get(ASSIGNMENTS_ENDPOINTS.BASE, filters, token);
...
export async function getAssignmentById(id, token) {
  try {
    const data = await apiClient.get(ASSIGNMENTS_ENDPOINTS.BY_ID(id), {}, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
...
export async function getMyAssignments(token) {
  try {
    const data = await apiClient.get(ASSIGNMENTS_ENDPOINTS.MY_ASSIGNMENTS, {}, token);
    return { success: true, data };
  } catch (error) {
    return { success: false, error: error.message };
  }
