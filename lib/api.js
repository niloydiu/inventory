const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://inv-api.tutorsplan.com/api/v1";

async function apiRequest(endpoint, options = {}) {
  const { method = "GET", body, token } = options;

  // If a token wasn't explicitly provided, try to read it from localStorage
  // (set by the auth flow). This keeps callers simple — they don't need to
  // manually pass the token after login.
  let finalToken = token;
  if (!finalToken && typeof window !== "undefined") {
    finalToken = localStorage.getItem("inventory_auth_token") || undefined;
  }
  ...
  if (!response.ok) {
    ...
    const errorMessage =
      error.message ||
      error.detail ||
      (error.errors && error.errors.length > 0 ? error.errors[0].msg : null) ||
  ...
  // If server uses the wrapper { success: boolean, data: ... }, unwrap it
...
export const itemsApi = {
  ...
  getById: (id, token) => apiRequest(`/items/${id}`, { token }),
  ...
  getCategories: (token) => apiRequest("/items/categories", { token }),
};
...
export const assignmentsApi = {
  ...
  getById: (id, token) => apiRequest(`/assignments/${id}`, { token }),
  ...
  getMyAssignments: (token) => apiRequest("/assignments/user/me", { token }),
};
...
export const livestockApi = {
  getAll: (token, params) => {
    ...
    if (params?.health_status)
      searchParams.append("health_status", params.health_status);
    if (params?.search) searchParams.append("search", params.search);
    const query = searchParams.toString() ? `?${searchParams}` : "";
    return apiRequest(`/livestock${query}`, { token });
  },

  getById: (id, token) => apiRequest(`/livestock/${id}`, { token }),
...
export const feedsApi = {
  ...
  getById: (id, token) => apiRequest(`/feeds/${id}`, { token }),
...
export const locationsApi = {
  getAll: (token) => apiRequest("/locations", { token }),

  getById: (id, token) => apiRequest(`/locations/${id}`, { token }),
...
export const dashboardApi = {
  getStats: (token) => apiRequest("/dashboard/stats", { token }),

  getSummary: (token) => apiRequest("/dashboard/summary", { token }),

  getRestocking: (token) => apiRequest("/dashboard/restocking", { token }),

  getRecentItems: (token) => apiRequest("/dashboard/recent-items", { token }),
};
...
export const reportsApi = {
  ...
  getAssignedItems: (token) => apiRequest("/reports/assigned-items", { token }),
...
export const reservationsApi = {
  getAll: (token) => apiRequest("/reservations", { token }),
...
export const approvalsApi = {
  ...
  getPending: (token) => apiRequest("/approvals/pending", { token }),
...
export const exportApi = {
  exportItemsCSV: (token) => `${API_BASE_URL}/export/items/csv?token=${token}`,
